...
for outtext in reviewed_files:
    ...
    for a, b in itertools.combinations(match, 2):
        ...
        if date_diff.days > 92:
            error_statement = "Date difference too high: " + str(date_diff.days) + " in file: " + filename + "Needs to be under X months/ X days" + " Dates: " + " ".join(str(x) for x in match)
            raise ValueError("Date diff too high", error_statement)